...
# Uncomment the folowing line to disable bi-weekly auto-update checks.
...
# alias sshfcs="ssh nextshiftrobotics@10.104.194.12"
#
#
...
# auto-start tmux
if [[ -z "$TMUX" ]] ;then
    ID="`tmux ls | grep -vm1 attached | cut -d: -f1`" # get id of attached session
    if [[ -z "$ID" ]] ;then
        tmux -2 new-session
    else
